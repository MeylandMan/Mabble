...
#include <filesystem>
	...
	#define MABBLE_INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { LOGGER##type##ERROR(msg, __VA_ARGS__); MABBLE_DEBUGBREAK(); } }
	...
	#define MABBLE_CORE_ASSERT(...) MABBLE_EXPAND_MACRO( MABBLE_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_CORE_, __VA_ARGS__) )
#else
