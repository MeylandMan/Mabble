...
#include <filesystem>
	...
	#define _INTERNAL_ASSERT_IMPL(type, check, msg, ...) { if(!(check)) { LOGGER_ERROR(msg, __VA_ARGS__); MABBLE_DEBUGBREAK(); } }
	#define _INTERNAL_ASSERT_WITH_MSG(type, check, ...) _INTERNAL_ASSERT_IMPL(type, check, "Assertion failed: {0}", __VA_ARGS__)
	#define _INTERNAL_ASSERT_NO_MSG(type, check) _INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", MABBLE_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).filename().string(), __LINE__)
